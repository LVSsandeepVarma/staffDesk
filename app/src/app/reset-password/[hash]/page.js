...
export default function PasswordHash() {
    ...
    useEffect(()=>{
      const verifyHash = async()=>{
        try{
          ...
          const response = await axios.post("https://admin.tradingmaterials.com/api/staff/verify/hash", {
            hash: hash
          })
  ...
  return (
      ...
      shouldAutoFocus
        ...
        inputType='text'
        inputStyle={{width:"3rem", height:"3rem", margin: "0 1rem", fontSize : "2rem", borderRadius:"4px", border:"1px solid"}}
        onChange={setOtp}
      />
       {responseError.length ? <p className='text-red-900'>{responseError}</p> : ""}

      <button className="flex justify-center w-[100%] mt-5 btn btn-block btn-primary btn-lg font-weight-medium auth-form-btn" onClick={()=>{handleOtpVerify()}} > Verify OTP</button>
