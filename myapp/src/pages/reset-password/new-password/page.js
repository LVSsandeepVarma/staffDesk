    ...
    const handleSubmit = async(values) => {
        ...
        try{
            ...
            if(values.password != values.confirmPassword){
                ...
                const response= await axios.post("https://admin.tradingmaterials.com/api/staff/reset/password",{hash: hash,"password":values.password, "confirm_password" : values.confirmPassword})
                // if(response.)
                        ...
                        <div className="row w-100 mx-0 h-[100vh] bg-[#edeef2]">
                            <div className="col-lg-4 mx-auto flex items-center w-[100%] flex justify-center !p-0">
                                <div className="auth-form-light text-start py-5 px-4 px-sm-5 bg-[#fff]">
                                    ...
                                    <h6 className="fw-light text-sm">Enter new passsword to continue.</h6>
                                    <Formik initialValues={initialValues} validationSchema={validationSchema} onSubmit={(values) => handleSubmit(values)}>
                                        <Form className="pt-3" >
                                            <div className="form-group mb-[1.5rem]">
                                                <Field className="form-control form-control-lg text-base bg-transparent" type="password" id="password" name="password" placeholder="new password" />
                                                <ErrorMessage className='text-red-900' name="password" component="div" />
                                            </div>
                                            <div className="form-group mb-[1.5rem]">
                                                <Field className="form-control form-control-lg text-base bg-transparent" type="password" id="confirmpassword" name="confirmPassword" placeholder="re enter your new passsword" />
                                                <ErrorMessage className='text-red-900' name="confirmPassword" component="div" />

                                            </div>
                                            {responseError.length ? <p className='text-red-900'>{responseError}</p> : ""}
                                            <div className="mt-3">
                                                <button type="submit" className="btn btn-block btn-primary btn-lg font-weight-medium auth-form-btn" href="../../index.html">Submit new password</button >
                                            </div>
                                        </Form>
                                    </Formik>
                                </div>
                            ...
                            <p className='flex justify-center text-2xl text-red-900 '>please do not refresh the page</p>
                        </div>
                    </div>
                </div>
            </div>
