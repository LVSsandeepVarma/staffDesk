...
const DispatchedTable = () => {
  ...
  const [filteredTable, setFilteredTable] = useState([]);
    ...
    const handleSearchChange = (event) => {
      ...
      // console.log(item)
        item?.email?.toLowerCase().startsWith(event.target.value.toLowerCase()) || item?.order_number?.includes(event.target.value?.toLowerCase()) || item?.customer?.first_name?.startsWith(event.target.value?.toLowerCase())
      ); 
       ...
       <Table  striped bordered hover>
          ...
          {currentData?.length >0 ? (<tbody>
            {currentData.map((row) => (
              <tr key={row.id}>
                  <td className="">
                            <h5 className="text-[#25378b] mb-1">{row?.customer?.first_name}{" "}{row?.customer?.last_name}</h5>
