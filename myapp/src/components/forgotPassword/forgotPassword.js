...
export default function Forgot_Password() {
    ...
    const handleSubmit = async(values) => {
        ...
        try{
            const response = await axios.post(" https://admin.tradingmaterials.com/api/staff/reset-password-link", {
                email:values?.email
            })
            setResponseError("")
    ...
    return (
        <main>
            {loaderState && <Loading/>}
            <EmailModal show={showSuccessModal} setShowSuccessModal={setShowSuccessModal} />
                        ...
                        <div className="row w-100 mx-0 h-[100vh] bg-[#edeef2]">
                            <div className="col-lg-4 mx-auto flex items-center w-[100%] flex justify-center !p-0">
                                <div className="auth-form-light text-start py-5 px-4 px-sm-5 bg-[#fff]">
                                    ...
                                    <h4 className="text-base	text-blue-950 font-extrabold	">Reset Password</h4>
                                    <h6 className="fw-light text-sm">Please enter your registered email</h6>
                                    <Formik initialValues={initialValues} validationSchema={validationSchema} onSubmit={(values) => handleSubmit(values)}>
                                        <Form className="pt-3" >
                                            ...
                                            <div className="mt-3">
                                                <button type="submit" disabled={disabled}  className="btn btn-block btn-primary btn-lg font-weight-medium auth-form-btn">Send Password Reset Link</button >
                                            </div>
